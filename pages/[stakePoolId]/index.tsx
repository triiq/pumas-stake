    ...
    const txs: (Transaction | null)[] = await Promise.all(
      stakedSelected.map(async (token) => {
        try {
          ...
          notify({
            message: `${e}`,
            description: `Failed to unstake Puma ${token?.stakeEntry?.pubkey.toString()}`,
            type: 'error',
          })
          return null
        }
  ...
  async function handleStake() {
    ...
    for (let step = 0; step < unstakedSelected.length; step++) {
      try {
        ...
        notify({
          message: `Failed to stake Puma ${unstakedSelected[
            step
    ...
    const txs: (Transaction | null)[] = await Promise.all(
      unstakedSelected.map(async (token) => {
        try {
          ...
          notify({
            message: `Failed to unstake Puma ${token?.stakeEntry?.pubkey.toString()}`,
            description: `${e}`,
            type: 'error',
          })
          return null
          ...
          !wallet.connected && (
            <div
              className={`mx-5 mb-5 cursor-pointer rounded-md border-[1px]  p-4 text-center text-lg font-semibold ${stakePoolMetadata?.colors?.accent &&
                stakePoolMetadata?.colors.fontColor
                ? ''
                : 'border-yellow-500 bg-yellow-500 bg-opacity-15'
                }`}
              style={
                ...
                <div className="flex min-w-[200px] flex-col text-lg">
                  {!rewardMintInfo.isFetched || !rewards.data ? (
                    <div className="relative flex h-10 w-full items-center justify-center">
                      ...
                      <div className="absolute w-full animate-pulse items-center justify-center rounded-lg bg-white bg-opacity-15 p-5"></div>
                    </div>
                  ) : (
                    rewards.data && (
                      <>
              ...
              <MouseoverTooltip title="Click on tokens to select them">
                ...
                >
                  ...
                  <span className="my-auto">
                    Stake Pumas ({unstakedSelected.length})
                  </span>
                </button>
              </MouseoverTooltip>
            </div>
            ...
            <div className="mt-2 flex flex-row-reverse">
              ...
              >
                ...
                >
                  ...
                  <span className="my-auto">
                    Unstake Pumas ({stakedSelected.length})
                  </span>
                </button>
              </MouseoverTooltip>
              {rewardDistributorData.data &&
